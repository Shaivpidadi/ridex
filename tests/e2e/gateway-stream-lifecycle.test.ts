...
describe("gateway stream lifecycle", () => {
  ...
  test("incomplete terminal output preserves process status without replaying effects", async () => {
    const root = createFixtureRoot("terminal-incomplete-output");
    const tracePath = join(root.root, "trace.log");
    const effectPath = join(root.workspace, "command-effect.txt");
    const callId = "terminal_incomplete_output_1";
    let observedFailure = "";
    let step = 0;
    const gateway = startGateway((body) => {
      switch (step++) {
    ...
    try {
      ...
      expect(gateway.requestCount()).toBe(2);
      expect(readFileSync(effectPath, "utf8")).toBe("effect\n");
      expect(observedFailure).toContain("could not be read completely");
