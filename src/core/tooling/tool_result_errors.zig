...
fn filesystemAccessDeniedSuggestion() []const u8 {
    if (builtin.os.tag == .macos) {
        return "Do not retry this path unchanged or propose a symlink. ridex permissions cannot override the operating system. If the path is in a protected folder such as Desktop, Documents, or Downloads, ask the user to grant the terminal app Files and Folders or Full Disk Access. Otherwise, ask the user to correct OS filesystem permissions or move/copy the project to an accessible location.";
    }
    return "Do not retry this path unchanged or propose a symlink. ridex permissions cannot override the operating system. Ask the user to correct OS filesystem permissions or move/copy the project to an accessible location.";
}
...
test "filesystem access denial JSON preserves recovery details" {
    ...
    for (errors) |err| {
        ...
        try std.testing.expect(std.mem.find(u8, suggestion, "ridex permissions") != null);
        if (builtin.os.tag == .macos) {
