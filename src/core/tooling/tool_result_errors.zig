...
pub fn toolPermissionDenialReason(output: []const u8) ?types.ToolPermissionDenialReason {
    ...
    const reason = std.meta.stringToEnum(types.ToolPermissionDenialReason, reason_value) orelse return null;
    if (std.mem.eql(u8, type_value, "tool_permission_denied")) return switch (reason) {
        .review_caution, .review_unavailable => null,
        else => reason,
    };
    if (std.mem.eql(u8, type_value, "tool_review_held")) return switch (reason) {
        .review_caution, .review_unavailable => reason,
        else => null,
    };
    return null;
}
...
test "review hold JSON distinguishes caution and unavailable from permission denial" {
    ...
    try std.testing.expect(toolPermissionDenialReason("{\"error\":{\"type\":\"tool_review_held\",\"reason\":\"auto_denied\"}}") == null);
    try std.testing.expect(toolPermissionDenialReason("{\"error\":{\"type\":\"tool_permission_denied\",\"reason\":\"review_caution\"}}") == null);
}
