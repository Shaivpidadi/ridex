//! `ridex replay <tape>` implementation.
//!
...
fn runWithOutput(alloc: Allocator, args: []const [:0]const u8, output: anytype) !u8 {
    ...
    const file = std.Io.Dir.cwd().openFile(io_mod.getIo(), opts.path, .{}) catch |err| {
        return replyFormattedError(alloc, output, opts.json, @errorName(err), 512, "ridex replay: cannot open {s}: {s}\n", .{ opts.path, @errorName(err) }, "ridex replay: open failed\n");
    };
    var file_mut = file;
    defer file_mut.close(io_mod.getIo());

    const bytes = io_mod.readFileToEnd(alloc, &file_mut, 64 * 1024 * 1024) catch |err| {
        return replyFormattedError(alloc, output, opts.json, @errorName(err), 256, "ridex replay: read failed: {s}\n", .{@errorName(err)}, "ridex replay: read failed\n");
    };
    defer alloc.free(bytes);

    var parser = record_tape.Parser.init(bytes) catch |err| {
        return replyFormattedError(alloc, output, opts.json, @errorName(err), 256, "ridex replay: bad tape: {s}\n", .{@errorName(err)}, "ridex replay: bad tape\n");
    };
    ...
    if (opts.frames_dir) |dir| {
        prepareFramesDir(alloc, dir) catch |err| {
            return replyFormattedError(alloc, output, opts.json, @errorName(err), 512, "ridex replay: cannot prepare frames dir {s}: {s}\n", .{ dir, @errorName(err) }, "ridex replay: cannot prepare frames dir\n");
        };
    }
    ...
    while (true) {
        ...
        if (opts.frames_dir) |dir| {
            writeFrameArtifacts(alloc, dir, frame_count, frame, elapsed_ms, grid, markers.items) catch |err| {
                return replyFormattedError(alloc, output, opts.json, @errorName(err), 512, "ridex replay: cannot write frame artifacts to {s}: {s}\n", .{ dir, @errorName(err) }, "ridex replay: cannot write frame artifacts\n");
            };
        }
    }
    ...
    if (ignored_incomplete_tail) {
        try output.writeStderr("ridex replay: ignored incomplete final tape frame\n");
    }

    if (opts.frames_dir) |dir| {
        writeFramesManifest(alloc, dir, parser.header, frame_count, resize_count, stdout_bytes) catch |err| {
            return replyFormattedError(alloc, output, opts.json, @errorName(err), 512, "ridex replay: cannot write frames manifest to {s}: {s}\n", .{ dir, @errorName(err) }, "ridex replay: cannot write frames manifest\n");
        };
    }
    ...
    if (opts.golden_path) |out_path| {
        var out_file = std.Io.Dir.cwd().createFile(io_mod.getIo(), out_path, .{ .truncate = true }) catch |err| {
            return replyFormattedError(alloc, output, opts.json, @errorName(err), 256, "ridex replay: cannot write {s}: {s}\n", .{ out_path, @errorName(err) }, "ridex replay: write failed\n");
        };
...
fn replyParseError(
    ...
    const msg = switch (err) {
        Error.MissingTapePath => "ridex replay: missing tape path\nusage: ridex replay <tape> [--frames] [--json] [--golden <path>] [--frames-dir <path>]\n",
        Error.TooManyArgs => "ridex replay: too many positional arguments\n",
        Error.UnknownFlag => "ridex replay: unknown flag\n",
        Error.MissingGoldenPath => "ridex replay: --golden requires a path\n",
        Error.MissingFramesDirPath => "ridex replay: --frames-dir requires a path\n",
        else => "ridex replay: argument error\n",
    };
...
test "json replay recovers an incomplete final frame through stderr" {
    ...
    try testing.expectEqualStrings("ridex replay: ignored incomplete final tape frame\n", capture.stderr.written());
}
...
test "run missing tape path returns current stderr" {
    ...
    try testing.expect(std.mem.startsWith(u8, capture.stderr.written(), "ridex replay: missing tape path\n"));
}
...
test "run malformed tape returns bad tape stderr" {
    ...
    const exit_code = try runCaptured(alloc, &.{tape_arg}, &capture);
    try testing.expectEqual(@as(u8, 1), exit_code);
    try testing.expect(std.mem.startsWith(u8, capture.stderr.written(), "ridex replay: bad tape"));
}
