...
const subagent_create_schema = model_tool_schema.ObjectSchema{
    .properties = &.{
        ...
        .{ .name = "profile", .json_type = .string, .bounds = &.{ .min_length = 1, .max_length = subagent_domain.max_profile_bytes }, .description = "Optional global profile from ~/.fx/agents/<name>.md. Explicit create fields override profile defaults." },
...
test "built-in model-facing tool contract stays byte exact" {
    ...
    try std.testing.expectEqualStrings(
        "7cb22b4b108523272a1bd402455aeb712fc8d4b3e981546a0e0d634a27d4398e",
        &actual_hex,
    );
}
